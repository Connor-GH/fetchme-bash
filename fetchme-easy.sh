...
LC_ALL=C # No unicode = go faster
LANG=C   # This line and the above taken from neofetch
OS=$(lsb_release -d | awk '/Description:/{print $2}')
...
OS:${c2} `lsb_release -d | awk '/Description:/{print $2" "$3}'` `uname -m`${c1}
...
GPU: ${c2}`lspci | awk '/VGA/{$1=$2=$3=$4="";print $0}' | cut -c 5-`${c1}
Memory: ${c2}`free -h --si | awk '/Mem:/{print $3 "/" $2}'`
${c2}"
